...
its own sake — a self-interpreter, in `meta.42` — and carries a weaker status
than the rest, stated where it is claimed and again in section 8.
...
pushed through — but it is not written out here. What stands behind it is that
every claim below was run against `rel42.run` on the same term, so the oracle is
the implementation the interpreter is a model of. Section 8 restates this.
...
- **Section 7 is checked, not proved.** Proposition 17 asserts that `meta.42`
  ...
  checked against `rel42.run` over a corpus of terms and values covering every
  constructor, both directions and recursion through the environment, which is
  a good oracle — it is the implementation the interpreter models — and is
  still a finite set of cases. Sections 3 to 6 do not depend on any of it.
