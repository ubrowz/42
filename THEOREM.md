...
5.1), and it is not mechanised in a proof assistant; section 8 is precise about
both. Every construction in sections 4 and 5 is in `theorem.42` or `tm.42`, and
is exercised by `tests/test_rel42.py`. Section 7 is a corollary developed for
its own sake — a self-interpreter, in `meta.42`, exercised by the same file —
and carries a weaker status than the rest, stated where it is claimed and again
in section 8.
...
7. [Self-interpretation](#7-self-interpretation)
...
## 7. Self-interpretation

Section 5 simulated a Turing machine. This section asks the sharper question:
can 42 interpret *42*?

Theorem 14 already answers it. Fix a set of definitions `Γ`. The relation

```
    Eval_Γ  =  { ((⌜t⌝, ⌜v⌝), (⌜t⌝, ⌜w⌝))  :  w ∈ ⟦t⟧_Γ(v) }
```

is r.e. — enumerate derivations — so some 42 program denotes it, and nothing
further needs proving. What the completeness proof gives, though, is a program
built out of `halts_M`: a Turing machine simulating an interpreter. The direct
construction is worth having instead, because it says something about the dagger
that the simulation cannot, and because it is short.

It is `meta.42`, and the claim it exists to support is Theorem 19.

### 7.1 The encoding

**Values.** Every element of `𝕍` is a finite tree over the unit, the two
injections and the pair (§2.1), so one type holds all of them:

```
    val = μV. 1 + (V + (V + (V × V)))
```

Write `⌜v⌝` for the evident injection `𝕍 → V(val)`. It is a bijection onto
`V(val)`, and both directions are 42 programs.

**Terms.** Seven constructors, matching `Term` in `rel42/core.py`:

```
    tm = μT. (pname × flag)        a primitive, and whether it is inverted
           + ((nat × flag)         a reference into the environment
           + ((T × T)              t ; u
           + ((T × T)              t | u
           + ((T × T)              t + u
           + ((T × T)              t * u
...
with `pname` the thirteen-way sum naming the primitives of §2.2 and `flag =
1 + 1`. `Fun`, `App` and `Var` are absent, and their absence is not a gap: by
§2.4 they are eliminable by substitution, and `expand_env` eliminates them
before the evaluator sees anything, so the seven above are exactly the language
the Python evaluator runs.

**Environments and states.** `env = list tm`, and a `Ref` carries an index into
it, so mutually recursive groups are encodable. A **state** is

```
    state = env × (tm × val)
```

and the interpreter is a relation `eval : state <-> state`.

Carrying the term inside the state, rather than currying it away, is the design
decision the rest of the section rests on. 42 has no binder over a value, so
there is no currying to be had — but the point is not the workaround. It is that
a state carries its own program, so `eval!` has something to be the converse
*of*.

### 7.2 The interpreter

Two things are worth seeing before the details, and both are consequences of
interpreting a language in itself rather than in something else.

**Nothing is implemented that could be inherited.** `;` is interpreted by `;`,
`|` by `|`, `+` by `+`, `*` by `*`, and `^` by `^`. The star case is the whole
of it:

```
def evstar = toframe! ; eval^ ; toframe ; (tagstar * id)
```

No fixpoint machinery, no trampoline, no halting test — for the same reason
`tm.42` needs no loop construct. It works because `eval` fixes the environment
and the term, so the meta-level closure ranges over exactly the value
component, which is the object-level closure.

The thirteen primitives are inherited too, including the two that matter most:
object-level equality is meta-level `copy!` and object-level nondeterminism is
meta-level `join!`. So §2.3's remark survives a level up — every set larger than
a singleton in the interpreted language still traces back to one primitive, and
it is the same one.
...
> **Proposition 17 (Adequacy).** For every core term `t`, environment `Γ` and
> value `v`,
>
> ```
>     ⟦eval⟧(⌜Γ⌝, (⌜t⌝, ⌜v⌝))  =  { (⌜Γ⌝, (⌜t⌝, ⌜w⌝))  :  w ∈ ⟦t⟧_Γ(v) }.
> ```

Two claims at once: the environment and term components are *preserved*, and the
value component is the image. The first is what makes the second sayable.

**Status: checked, not proved.** The induction on `t` is routine — one case per
constructor, each of which is the corresponding clause of §2.3 with the encoding
pushed through — but it is not written out here. What stands behind it is
`TestSelfInterpretation` in `tests/test_rel42.py`, which runs this proposition,
Corollary 18 and Theorem 19 against `rel42.run` on the same term — so the oracle
is the implementation the interpreter is a model of — over every constructor,
both directions and recursion through the environment. Section 8 restates the
limits of that.
...
`metatoggle` is prelude.42's `toggle = not^` reaching the interpreter as data;
`metaalt` is `id | not`, the branch that needs no exit assertion. Nondeterminism
survives the encoding: one input, two answers, at both levels.

### 7.4 The dagger

Because `eval` preserves the environment and the term, so does its converse.
That single fact is the section's point.

> **Corollary 18 (Backward adequacy).** `⟦eval!⟧(⌜Γ⌝, (⌜t⌝, ⌜w⌝)) = { (⌜Γ⌝,
> (⌜t⌝, ⌜v⌝)) : v ∈ ⟦t!⟧_Γ(w) }.`

*Proof.* Proposition 17 and `⟦t!⟧ = ⟦t⟧°` (§2.3). ∎

There is no second construction and no second induction. The backward
interpreter is the forward one, read by a `dagger` that was total before anyone
asked it to interpret anything:
...
Now the statement that does *not* come free. `meta.42` also defines `dag`, the
syntactic dagger of §2.3 written in 42 — seven cases, of which only two carry
content: `swapprod` is `Seq(s,u) ↦ Seq(u†,s†)`, the contravariance, and
`swapsum` is `Prim(n,i) ↦ Prim(n,¬i)`, the inv flag.

> **Theorem 19.** `⟦eval!⟧ = ⟦(id * (dag * id)) ; eval ; (id * (dag * id))⟧`.

Equivalently: daggering the interpreter and daggering the encoded program agree.
`dag` is an involution, so the conjugation is well formed in both directions.

**Why this is the load-bearing statement, and the defining law is not.** It is
tempting to check a self-interpreter by confirming

```
    x ∈ ⟦eval⟧(y)   ⟺   y ∈ ⟦eval!⟧(x),
```

and that does hold. It holds because `eval` is a 42 term, not because it is
*that* one: substitute `add` and it holds equally, and it would hold of an
interpreter that computed something else entirely. The law is a conservation
property of the syntax, and a construction cannot be validated by a property
every construction has.

Theorem 19 is where the content is. It says the *program* `dag` computes the
converse the law promises. An interpreter with a broken `dag` satisfies the law
and fails Theorem 19, which is the only place it would be caught.

The same distinction separates this from Proposition 17: adequacy says `eval`
denotes what its argument denotes, Theorem 19 says the two ways of reversing it
coincide, and neither implies the other. Note the shape of the pair — the total,
structural `dagger` of §2.3 is *internalised* here as a 42 program, and Theorem
19 says the internalisation is faithful. That is what a language whose class is
closed under converse buys, stated at one remove.
...
nth)`. The two denote the same relation. But the dagger of the short one is
`(id * nth!) ; copy!`, and `nth!` on a term enumerates every environment
containing it, which does not terminate; the dagger of the recursive one
*checks* `Γ[i] = t` against an index and an environment it already has. Same
morphism of Rel, and only one of the two runs. That is a fact about the
evaluator's strategy, not about `⟦·⟧`, and it is the sharpest illustration in
this document of the gap §8 insists on.

**The type checker meets its match.** `meta.42`'s 85 definitions all type — `42
...
`dagger`'s two presentations of the same infinite tree print differently. The
property holds — the test now falls back to the unifier when the strings differ
— but it is a concrete instance of the non-canonicity of equirecursive types
that §2.1 chose and the README lists as a limitation.
...
- **None of this is mechanised.** No proof assistant has seen it. The
  ...
  proof.
- **Section 7 is checked, not proved.** Proposition 17 asserts that `meta.42`
  interprets the core, and its induction is routine but is not written out;
  Theorem 19 is likewise stated on the strength of having been run. Both were
  checked by `TestSelfInterpretation` in `tests/test_rel42.py`, over a corpus
  of terms and values covering every constructor, both directions and recursion
  through the environment. That is a good oracle — it is the implementation the
  interpreter models — and is still a finite set of cases. Sections 3 to 6 do
